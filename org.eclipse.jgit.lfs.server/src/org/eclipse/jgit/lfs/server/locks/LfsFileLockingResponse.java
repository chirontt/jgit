 ...
 * <p>
 * See the <a href=
 * "https://github.com/git-lfs/git-lfs/blob/main/docs/api/locking.md">LFS File
 * Locking API specification</a>
 *
 */
public interface LfsFileLockingResponse {
	...
	/**
	 * Describes the current active locks which are owned by the current user
	 * ("ours") and which are owned by other users ("theirs"), for verification
	 * purpose.
	 */
	class LocksToVerify {
		List<Lock> ours;
	...
	/**
	 * Describes the error to be returned by the LFS file locking API, when
	 * trying to create a new LFS lock but the lock already exists
	 */
