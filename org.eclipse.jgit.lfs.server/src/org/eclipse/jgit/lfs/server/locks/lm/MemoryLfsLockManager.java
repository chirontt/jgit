...
public class MemoryLfsLockManager implements LockManager {
	...
	/**
	 * Construct a memory lock manager for given git repository.
	 *
	 * @param repoPath
	...
	public Locks listLocks(String path, String id, String cursor, int limit,
		...
		lockCache.values().stream()
				.filter(lock -> lockMatched(lock, path, refspec))
	...
	public LocksToVerify listLocksToVerify(String refName, String username,
		...
		lockCache.values().stream()
				.filter(lock -> lockMatched(lock, null, refName))
